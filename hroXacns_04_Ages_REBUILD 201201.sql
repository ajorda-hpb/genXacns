 /*Change Notes for hroXacns_Age logic....
...
Missing Index Details from hroXacns_Age CREATE live 191106.sql - sage.master (HPB\ajorda (51))
The Query Processor estimates that implementing the following index could improve the query cost by 36.3149%.

USE [ReportsView]
GO
CREATE NONCLUSTERED INDEX [ncidx_hroXacns_flow_QtyItemU]
ON [dbo].[hroXacns] ([flow])
INCLUDE ([Qty],[Item],[u])
GO
...
-------------------------------------------------------------[
/*
drop table if exists ReportsView.dbo.hroXacns_Ages
create table ReportsView.dbo.hroXacns_Ages
	(Loc char(5) not null
	...
	,constraint PK_hroXacnsAges primary key(Loc,Item,ItemCode,u));
*/


truncate table ReportsView.dbo.hroXacns_Ages
...
,maxInU as(
	...
	from ReportsView..hroXacns n inner join ReportsView..hroXacns_SetAdjs sa 
...
insert into ReportsView.dbo.hroXacns_Ages
