...
/****** Object:  StoredProcedure [dbo].[CDC_genXacns_01_Items_update]    Script Date: 2/24/2021 2:36:09 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE procedure [dbo].[CDC_genXacns_01_Items_update]
as

--CHANGE LOG------------------------------------------------------------------------------------
--12/04/19: sp version of file: genXacns_Items UPDATE job 191122.sql
...
----List of items, old and new, being included in genXacns--------------------
...
where pm.ItemCode not in (select ItemCode from #excludes)
	and (ltrim(rtrim(pm.SectionCode)) in ('BLANK','BOOKLIGHT','BOOKMARK','HEADPHONES','NOTECARDS','PUZZLES','EYEGLASSES','METALSIGNS','DRINKWARE','PLUSH')
		...
		or (/*pm.Reorderable = 'Y' and*/ pm.PurchaseFromVendorID in ('TEXASBKMNA','TEXASBKMNB','TEXASBKNON','TEXASSTATI','TEXASBKPUZ','TEXASBKUPC'))
		or (pm.UserChar15 = 'TTB' and pm.CreateDate > @minDate) 
		or pm.PurchaseFromVendorID in ('IDAURORA ','IDBENDONPU','IDBKSALESI','IDBOOKDEPO','IDBRYBELLY','IDC&DVISIO','IDCRAZART','IDCROWNB&C'
...
----Transaction 1--------------------------------
--Updates to existing genXacns_Items-------------[
-----------------------------------------------[
BEGIN TRY
	begin transaction

		update ReportsView.dbo.genXacns_Items
		set Item = src.Item
			,RptIt = src.RptIt
			...
			,riUPC = src.riUPC
			,LastUpdate = getdate()
		from ReportsView.dbo.genXacns_Items tp 
...
----Transaction 2--------------------------------
--Inserting NEW items genXacns_Items-------------[
-----------------------------------------------[
BEGIN TRY
	begin transaction
	
	insert into ReportsView.dbo.genXacns_Items
	select it.ItemCode
		,it.Item
		,it.RptIt
		,it.riVendorID
		...
		,getdate()
	from #items it 
		left join ReportsView.dbo.genXacns_Items tp 
...
drop table if exists ReportsView.dbo.genXacns_Items 

create table ReportsView.dbo.genXacns_Items
	...
	,constraint PK_genXacnsItems primary key(ItemCode)
	);
-----------------------------------------------]
------------------------------------------------]
