...
/*
drop table if exists ReportsView.dbo.ngXacns_SetAdjs
create table ReportsView.dbo.ngXacns_SetAdjs
	(Loc char(5) not null
	...
	,constraint PK_ngXacnsSetAdjs primary key(Loc,ItemCode,crSet,nxIn))
*/

truncate table ReportsView..ngXacns_SetAdjs

--When ngXacns DOES have crSet & nxIn...
--    drop table if exists ReportsView.dbo.ngXacns_SetAdjs
;with fAdjQ_means as(
...
insert into ReportsView.dbo.ngXacns_SetAdjs
select fa.Loc,fa.ItemCode
...
from fAdjQ_prep fa
group by fa.Loc,fa.ItemCode,fa.crSet,fa.nxIn,fa.AdjQ ,fa.minInvQ,fa.minAInvQ


--create nonclustered index idx_ngXacnsSetAdjs_Loc on ReportsView.dbo.ngXacns_SetAdjs(Loc) include(ItemCode,crSet,nxIn,nAdjQ,fAdjQ)
