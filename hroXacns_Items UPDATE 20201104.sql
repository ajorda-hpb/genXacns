...
--CHANGE LOG------------------------------------------------------------------------------------
--12/04/19: sp version of file: hroXacns_Items UPDATE job 191122.sql
--02/21/20: Expanded list of sections to include Drinkware & Plush, which are now pull sections
--07/31/20: Separate version for HRO Vendor's Suggestions Tracking.
--01/05/21: Added IDBOOKDEPO to list of HRO Vendors
...
----List of items, old and new, being included in hroXacns--------------------
--This includes all currently reorderable HRO items for SuggestedOrders data
--& and items ever bought from HRO vendors, reoderable or not.
-----------------------------------------------------------------------------
    drop table if exists #set0_base
...
where pm.ItemCode not in (select ItemCode from #excludes)
    and pm.ItemCode not in (select ItemCode from ReportsView..ngXacns_Items)
	and (pm.PurchaseFromVendorID in ('IDAURORA ','IDBENDONPU','IDBKSALESI','IDBOOKDEPO','IDBRYBELLY','IDC&DVISIO','IDCRAZART','IDCROWNB&C'
                                    ,'IDCROWNPOI','IDCUDDLEBA','IDEUROGRA','IDFOUNDIMG','IDIGLOOBOO','IDKALANLPT','IDKIKKERLA','IDLBMAYASS'
                                    ,'IDMELISSA&','IDMODERNPU','IDNOSTIMAG','IDOUTOFPRI','IDPEEPERS','IDSALESCOR','IDTOYSMITH','IDUNEMPLOY'
                                    ,'IDUSPLAYIN','IDWISHPETS','IFDISCONFO','IFPROPERRE')
            or pm.VendorID in ('IDAURORA ','IDBENDONPU','IDBKSALESI','IDBOOKDEPO','IDBRYBELLY','IDC&DVISIO','IDCRAZART','IDCROWNB&C'
								,'IDCROWNPOI','IDCUDDLEBA','IDEUROGRA','IDFOUNDIMG','IDIGLOOBOO','IDKALANLPT','IDKIKKERLA','IDLBMAYASS'
								,'IDMELISSA&','IDMODERNPU','IDNOSTIMAG','IDOUTOFPRI','IDPEEPERS','IDSALESCOR','IDTOYSMITH','IDUNEMPLOY'
								,'IDUSPLAYIN','IDWISHPETS','IFDISCONFO','IFPROPERRE') )
        -- and pm.CreateDate > @minDate
        ;
...
----Transaction 1--------------------------------
--Updates to existing hroXacns_Items-------------[
-----------------------------------------------[
BEGIN TRY
	begin transaction

		update ReportsView.dbo.hroXacns_Items
		set Item = src.Item
			,RptIt = src.RptIt
			...
			,LastUpdate = getdate()
		--    select count(*)
		from ReportsView.dbo.hroXacns_Items tp 
...
----Transaction 2--------------------------------
--Inserting NEW items hroXacns_Items-------------[
-----------------------------------------------[
BEGIN TRY
	begin transaction
	
	insert into ReportsView.dbo.hroXacns_Items
	select it.ItemCode
		,it.Item
		,it.RptIt
		,it.riVendorID
	...
	--    select count(*)
	from #items it 
		left join ReportsView.dbo.hroXacns_Items tp 
...
drop table if exists ReportsView.dbo.hroXacns_Items 

create table ReportsView.dbo.hroXacns_Items
	...
	,constraint PK_hroXacnsItems primary key(ItemCode)
	);
-----------------------------------------------]
------------------------------------------------]
