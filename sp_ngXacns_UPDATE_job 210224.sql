...
/****** Object:  StoredProcedure [dbo].[CDC_ngXacns_UPDATE_daily_job]    Script Date: 2/24/2021 2:37:41 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE procedure [dbo].[CDC_ngXacns_UPDATE_daily_job]
as

----sp version of file: ngXacns UPDATE daily job 191203.sql ***12/04/19

----ngXacns - Transaction-level data for New Goods--------------------------------------------
----ajorda------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------
...
----to similar metics that are naturally calculated for uniquely ID'd SIPS/used goods.
----------------------------------------------------------------------------------------------
--CHANGE LOG----------------------------------------------------------------------------------
--12/04/19: sp version of file: ngXacns UPDATE daily job 191203.sql
...
----since the current update structure for the ngXacns data is a rolling two-day window.
...
----TODO: Edit the #wms stuff to use the wms_ils database instead of rILS_Data.
----And don't forget to collate database_default while you're there! >.<
...
----List of NEW items (no records in ngXacns)---------
...
select it.ItemCode
into #NewItems
from ReportsView..ngXacns_Items it 
...
from wms_ils..Shipment_Detail wms with(nolock)
	...
	inner join ReportsView..ngXacns_Items eq 
...
where wmsh.Actual_Ship_Date_Time < @eDate 
	and (wmsh.Actual_Ship_Date_Time >= @sDate or ni.ItemCode is not null)
group by right('0000000000'+ convert(varchar(10), wms.INTERNAL_SHIPMENT_NUM), 10)
	,eq.ItemCode
	,right('00000'+ convert(varchar(5), wms.SHIP_TO), 5)
...
from ReportsData..ShipmentDetail sd with(nolock)
	...
	inner join ReportsView..ngXacns_Items eq on sd.ItemCode = eq.itemCode
	left join #NewItems ni on eq.ItemCode = ni.ItemCode
...
from ReportsData..SR_Header srh with(nolock)
	...
	inner join ReportsView..ngXacns_Items eq on srd.ItemCode = eq.itemCode
	left join #NewItems ni on eq.ItemCode = ni.ItemCode
...
from rHPB_Historical.dbo.SalesItemHistory sh with(nolock)
	...
	inner join ReportsView..ngXacns_Items it on sh.ItemCode = it.ItemCode
...
from isis..Order_Monsoon om with(nolock)
	...
	inner join ReportsView..ngXacns_Items it on right(om.SKU,20) = it.ItemCode
...
from isis..Order_Omni od with(nolock)
	...
	inner join ReportsView..ngXacns_Items it on right(od.SKU,20) = it.ItemCode
	inner join ReportsData..ProductMaster pm with(nolock) on it.ItemCode = pm.ItemCode
	left join #NewItems ni on it.ItemCode = ni.ItemCode
...
from ReportsView..vw_StoreTransferDetail sh with(nolock)
	...
	inner join ReportsView..ngXacns_Items it on sh.DIPSItemCode=it.ItemCode
	left join #NewItems ni on it.ItemCode = ni.ItemCode
...
FROM ReportsData..SICC_Inventory_Adjustments sia with(nolock)
	...
	inner join ReportsView..ngXacns_Items it on sia.ItemCode=it.ItemCode
	left join #NewItems ni on it.ItemCode = ni.ItemCode
...
FROM ReportsData..CDC_Inventory_Adjustments cia with(nolock)
	...
	inner join ReportsView..ngXacns_Items it on cia.ItemCode=it.ItemCode
	left join #NewItems ni on it.ItemCode = ni.ItemCode
...
select xa.Loc
	...
	,it.Item
into #xacns_final 
from #xacns xa inner join ReportsView..ngXacns_Items it 
...
--------------------------------------------------------------------
----Updating & Inserting on ngXacns_ShRvDetail & ngXacns------------
--------------------------------------------------------------------

----Transaction 1----------------------------------------
--Update Item-level Receiving data ngXacns_ShRvDetail---[
-------------------------------------------------------[
BEGIN TRY
	...
	update ReportsView..ngXacns_ShRvDetail
	set RcvDate = src.RcvDate
		...
		,LastUpdate = @uDate
	from #ShpRcv src 
		inner join ReportsView..ngXacns_ShRvDetail tar
...
BEGIN TRY
	...
	;with ShpmtTots as(
		...
		from ReportsView..ngXacns_ShRvDetail sr with(nolock)
	...
	set ruRcvDt = src.ruRcvDt
		...
		,LastUpdate = @uDate
	from ShpmtTots src 
		inner join ReportsView..ngXacns_ShRvDetail tar with(nolock)
...
----Transaction 2--------------------------------
--Update Date/Qty/PshQty ngXacns_ShRvDetail-----[
-----------------------------------------------[
BEGIN TRY
	begin transaction
...
----Transaction 3--------------------------------
--Insert NEW records ngXacns_ShRvDetail---------[
-----------------------------------------------[
BEGIN TRY
	begin transaction
	set @uDate = getdate()
	...
	insert into ReportsView..ngXacns_ShRvDetail
	...
	-----------------------------------------------------------------------------------------------
		...
		,@uDate
	from #ShpRcv src 
		left join ReportsView..ngXacns_ShRvDetail tar 
		on tar.ItemCode = src.ItemCode 
			and tar.Loc = src.Loc 
			and tar.Shpmt = src.Shpmt
...
BEGIN TRY
	...
	update ReportsView..ngXacns
	set Date = src.Date
		,Qty = src.Qty
		,PshQty = src.PshQty
		,LastUpdate = @uDate
	from ReportsView..ngXacns tar
		inner join ReportsView..ngXacns_ShRvDetail src
		on tar.ItemCode = src.ItemCode 
		and tar.Loc = src.Loc 
		and tar.Shpmt = src.Shpmt
...
----Transaction 5--------------------------------
--Add new CDC/Drps/StSi records to ngXacns------[
-----------------------------------------------[
--   declare @uDate datetime
BEGIN TRY
	begin transaction
	...
	insert into ReportsView..ngXacns
	select src.Loc,src.ItemCode
		,0[SkuExt]
		,src.Shpmt,src.Date
		,src.Xacn,'in'
		...
		,null[crSet]
		,null[nxIn]
	from ReportsView..ngXacns_ShRvDetail src
		inner join ReportsView..ngXacns_Items it
		on src.ItemCode = it.ItemCode
		left join ReportsView..ngXacns tar 
		on tar.ItemCode = src.ItemCode 
			and tar.Loc = src.Loc 
			and tar.Shpmt = src.Shpmt
	where tar.ItemCode is null;
...
BEGIN TRY
	...
	from #xacns_final src left join ReportsView..ngXacns tar
...
----Transaction 7--------------------------------
--Update/Refresh u & InvQ fields in ngXacns-----[
-----------------------------------------------[
BEGIN TRY
	begin transaction
	...
	update ReportsView..ngXacns 
		set u = src.u
			,InvQ = src.InvQ
		from ReportsView..ngXacns tar inner join uInvQ_refresh src 
...
----Transaction 8--------------------------------
--Update/Recalc SetFlow markers in ngXacns------[
-----------------------------------------------[
BEGIN TRY
	begin transaction
