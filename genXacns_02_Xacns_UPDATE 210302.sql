...
----genXacns - Transaction-level data for New Goods--------------------------------------------
----ajorda------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------
--CHANGE LOG----------------------------------------------------------------------------------
--12/04/19: sp version of file: genXacns UPDATE daily job 191203.sql
...
----since the current update structure for the genXacns data is a rolling two-day window.
...
select it.ItemCode
into #NewItems
from ReportsView..genXacns_Items it 
...
from wms_ils..Shipment_Detail wms with(nolock)
	...
	inner join ReportsView..genXacns_Items eq 
...
select xa.Loc
	...
	,it.Item
into #xacns_final 
from #xacns xa inner join ReportsView..genXacns_Items it 
...
--------------------------------------------------------------------
----Updating & Inserting on genXacns_ShRvDetail & genXacns------------
--------------------------------------------------------------------

----Transaction 1----------------------------------------
--Update Item-level Receiving data genXacns_ShRvDetail---[
-------------------------------------------------------[
BEGIN TRY
	...
	update ReportsView..genXacns_ShRvDetail
	set RcvDate = src.RcvDate
		...
		,LastUpdate = @uDate
	from #ShpRcv src 
		inner join ReportsView..genXacns_ShRvDetail tar
...
----Transaction 1a-------------------------------------------
--Update Shipment-level Receiving data genXacns_ShRvDetail---[
-----------------------------------------------------------[
BEGIN TRY
	begin transaction	
	...
	;with ShpmtTots as(
		...
		from ReportsView..genXacns_ShRvDetail sr with(nolock)
	...
	set ruRcvDt = src.ruRcvDt
		...
		,LastUpdate = @uDate
	from ShpmtTots src 
		inner join ReportsView..genXacns_ShRvDetail tar with(nolock)
...
----Transaction 2--------------------------------
--Update Date/Qty/PshQty genXacns_ShRvDetail-----[
-----------------------------------------------[
BEGIN TRY
	begin transaction
...
----Transaction 3--------------------------------
--Insert NEW records genXacns_ShRvDetail---------[
-----------------------------------------------[
BEGIN TRY
	begin transaction
	set @uDate = getdate()
	...
	insert into ReportsView..genXacns_ShRvDetail
	...
	-----------------------------------------------------------------------------------------------
		...
		,@uDate
	from #ShpRcv src 
		left join ReportsView..genXacns_ShRvDetail tar 
		on tar.ItemCode = src.ItemCode 
			and tar.Loc = src.Loc 
			and tar.Shpmt = src.Shpmt
...
----Transaction 4--------------------------------
--Update calc fields in genXacns-----------------[
-----------------------------------------------[
...
BEGIN TRY
	...
	update ReportsView..genXacns
	set Date = src.Date
		,Qty = src.Qty
		,PshQty = src.PshQty
		,LastUpdate = @uDate
	from ReportsView..genXacns tar
		inner join ReportsView..genXacns_ShRvDetail src
		on tar.ItemCode = src.ItemCode 
		and tar.Loc = src.Loc 
		and tar.Shpmt = src.Shpmt
...
----Transaction 5--------------------------------
--Add new CDC/Drps/StSi records to genXacns------[
-----------------------------------------------[
--   declare @uDate datetime
BEGIN TRY
	begin transaction
	...
	insert into ReportsView..genXacns
	select src.Loc,src.ItemCode
		,0[SkuExt]
		,src.Shpmt,src.Date
		,src.Xacn,'in'
		...
		,null[crSet]
		,null[nxIn]
	from ReportsView..genXacns_ShRvDetail src
		inner join ReportsView..genXacns_Items it
		on src.ItemCode = it.ItemCode
		left join ReportsView..genXacns tar 
		on tar.ItemCode = src.ItemCode 
			and tar.Loc = src.Loc 
			and tar.Shpmt = src.Shpmt
	where tar.ItemCode is null;
...
BEGIN TRY
	...
	from #xacns_final src left join ReportsView..genXacns tar
...
----Transaction 7--------------------------------
--Update/Refresh u & InvQ fields in genXacns-----[
-----------------------------------------------[
BEGIN TRY
	begin transaction
	...
	update ReportsView..genXacns 
		set u = src.u
			,InvQ = src.InvQ
		from ReportsView..genXacns tar inner join uInvQ_refresh src 
...
----Transaction 8--------------------------------
--Update/Recalc SetFlow markers in genXacns------[
-----------------------------------------------[
BEGIN TRY
	begin transaction
